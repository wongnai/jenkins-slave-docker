...
if [ "$1" = 'swarm' ]; then
  ...
  exec java -Dfile.encoding=UTF-8 ${java_vm_parameters} -jar ${SWARM_CLIENT_JAR} ${jenkins_default_parameters} -master ${jenkins_master} ${swarm_node_name} ${jenkins_executors} ${jenkins_user} ${jenkins_swarm_parameters} ${jenkins_workdir}
elif [[ "$1" == '-'* ]]; then
  # Run the Swarm-Client with passed parameters.
  exec java $JAVA_OPTS -jar ${SWARM_CLIENT_JAR} "$@"
else
  exec "$@"
fi
